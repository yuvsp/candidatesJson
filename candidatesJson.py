...
for candidate in data:
    ...
    for i in range(len(orderd_list)):
        if i > 0:  # checking for gaps, starting from second workplace
            ...
            if gap.days > 1:
                ...
                gaps_for_candidate.append({"GapNumber":num_of_gaps,"Starting":former_end.strftime('%b/%d/%Y'),"Days":gap.days})
    ...
    # recreating updated Json
    add_gaps = {"gaps":gaps_for_candidate}
    candidate.update(add_gaps)
    updated_json_with_gaps.append(candidate)
